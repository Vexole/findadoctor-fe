...
export default function PatientAppointments() {
  ...
  if (patientAppointmentsQuery.isError)
    return <pre>{JSON.stringify(patientAppointmentsQuery.error)}</pre>;
  ...
  const editAppointment = (appointment: Appointment) => {
    getLocalStorage().appointment = JSON.stringify(appointment);
  ...
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'flex-end', margin: '1rem 2rem' }}>
        <Button colorScheme="facebook" onClick={() => bookAppointment()}>
          Book Appointment
        </Button>
      </div>
